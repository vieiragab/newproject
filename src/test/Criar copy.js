...
const ItemPost = (props) => { 
  return (
    <View>
      <Text>{props.item.titulo}</Text>
      <Text>{props.item.subtitulo}</Text>
      <Text>{props.item.conteudo}</Text>
      <Text>{props.item.data}</Text>
      <Text>{props.item.autor}</Text>
      <Button name="delete" size={32} 
                  onPress={()=>{props.onApagar(props.item);}}/>
      <Button name="edit" size={32} 
